...
class SwitchButton @JvmOverloads constructor(
   ...
   /** Button width and height shape ratio (0,1] Large adjustment is not recommended */
    ...
    /** Whether to display button shadow */
        ...
        when (MeasureSpec.getMode(finalWidthMeasureSpec)) {
            MeasureSpec.AT_MOST, MeasureSpec.UNSPECIFIED -> {
                finalWidthMeasureSpec = MeasureSpec.makeMeasureSpec(
                    (resources.getDimension(com.cooldev.base.R.dimen.dp_56) + paddingLeft + paddingRight).toInt(), MeasureSpec.EXACTLY)
            }
            MeasureSpec.EXACTLY -> {}
        }
    ...
    /**
     * Whether the current status is selected
     */
    ...
    /**
     * Set the selection status (the listener will be called back by default)
     */
    ...
    /**
     *Set selected state change monitoring
     */
