...
import com.dhug.example.base.aop.Log
import com.dhug.example.base.manager.ActivityManager
import com.dhug.example.base.other.DebugLoggerTree
import com.dhug.example.base.other.TitleBarStyle
import com.dhug.example.base.other.ToastLogInterceptor
import com.dhug.example.base.other.ToastStyle
import com.dhug.example.domain.usecase.PurchaseUseCase
import com.dhug.example.framework.alarm.AlarmScheduler
import com.dhug.example.utils.AppConfig
...
class AppApplication : Application(), Configuration.Provider {
    ...
    private fun connectPurchase(onDone: () -> Unit) {
        purchaseUseCase.startConnection(
            onConnected = {
                Timber.tag("LOG BILLING").d("CONNECTED")
                CoroutineScope(Dispatchers.IO).launch {
            ...
            onError = {
                Timber.tag("LOG BILLING").d("${it.message}")
                it.printStackTrace()
                onDone()
            }
        )
    ...
    private fun initAd(application: Application) {
        ...
        MobileAds.initialize(application) { _ ->
            Timber.d("AdManager", "Google Mobile Ads SDK initialized.")
